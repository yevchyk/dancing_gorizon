...
> The "quiet test"/sealed exam, the probe/final/now split, `binance_cutoffs.json`
...
0.75–2.5% while every sim/label assumed flat 0.75% → models counted wins what
...
1. **Labels vs honest cost**: win = directional move from entry (t+5m close on
   ...
   **+ med|funding| × h/480** (configs/binance_funding.json). No flat threshold
   anywhere. (User 2026-06-10: funding is ALWAYS in — "so the net would ignore
   pointless risk, only super-confidence of high growth".)
...
5. **FINAL = [T−5d, T] — UNTOUCHED.** No training, no tuning, no zone-picking,
   ...
   via the explorer's "🤫 quiet test" source (`data_secret.js`, generated by
...
- VERDICT (2026-06-12 ~02:00, user accepted): d8@20k probe tail p≥0.85 with a
  ...
  (2026-06-12 ~02:30): d8 budget LOCKED ≤15k ("maybe even 14000") for the final
...
0a. NEXT-TRAINING grid (user, 2026-06-12 night): extend horizons to ~540m —
   "the best short bets are somewhere out there" (far end of the grid). Caveat for the
   ...
   via TIMING, not probe zone-picks — "their quality from the very bottom" → candidate
...
0a''. NIGHT POCKETS (deep miner 2026-06-12 ~16:30, gates: n>=40, both halves
   green, >=2 models confirm; saved builds in folder "night probe 12.06"):
